...
# Require the **coffee-maker** module to get access to the compiler.
...
# Config
{REPL_PROMPT, REPL_PROMPT_MULTILINE, REPL_PROMPT_CONTINUATION} = CoffeeMaker
...
completeVariable = (text) ->
  ...
  if free?
    ...
    keywords = (r for r in CoffeeMaker.RESERVED when r[..1] isnt '__')
    candidates = vars
...
run = (buffer) ->
  ...
  try
    _ = global._
    returnValue = CoffeeMaker.eval "#{code}", {
      filename: 'repl'
      modulename: 'repl'
    }
